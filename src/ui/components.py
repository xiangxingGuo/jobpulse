...
def render_job_detail(detail: dict | None) -> None:
    ...
    st.write(f"**Title:** {detail.get('title') or 'N/A'}")
    st.write(f"**Company:** {detail.get('company') or 'N/A'}")
    st.write(f"**Location:** {detail.get('location_text') or 'N/A'}")
    st.write(f"**Scrape Status:** {detail.get('scrape_status') or 'N/A'}")

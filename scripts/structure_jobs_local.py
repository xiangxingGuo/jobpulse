...
from src.extractors.local_hf import PROMPT_VERSION, SCHEMA_VERSION, LocalHFExtractor
...
def main():
    ...
    ap.add_argument(
        "--only-missing", action="store_true", help="Only process jobs not yet structured"
    )
    args = ap.parse_args()
    ...
    for i, (job_id, title, company, location_text, description, url) in enumerate(rows, 1):
        ...
        ok += 1
        print(
            f"[{i}/{len(rows)}] ✅ {job_id} {data.get('role_category')} {data.get('work_mode')} conf={conf:.2f}"
        )
